...
export const AuthContextProvider = ({ children }) => {
  ...
  const login = async (inputs) => {
    try {
      const res = await axios.post("/api/auth/login", inputs); // Ensure this matches your backend route
  ...
  const register = async (inputs) => {
    try {
      await axios.post("/api/auth/register", inputs);  // No need to add localhost:5000, because of proxy setup
